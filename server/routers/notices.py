...
async def list_notices(
    ...
    query = select(Notice).options(selectinload(Notice.case)).offset(skip).limit(limit)
...
async def get_notice(
    ...
    result = await db.execute(select(Notice).options(selectinload(Notice.case)).filter(Notice.id == notice_id))
