...
class User(Base):
    ...
    role = Column(Enum(UserRole, name="user_role", schema="app", create_type=False), nullable=False)
    designation = Column(String)
    department = Column(String)
    status = Column(Enum(UserStatus, name="user_status", schema="app", create_type=False), nullable=False, default=UserStatus.active)
...
class Case(Base):
    ...
    assigned_advocate_id = Column(UUID(as_uuid=True), ForeignKey("app.users.id", ondelete="SET NULL"), index=True)
    created_by = Column(UUID(as_uuid=True), ForeignKey("app.users.id", ondelete="SET NULL"))
...
class CaseArbitration(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    case_id = Column(UUID(as_uuid=True), ForeignKey("app.cases.id", ondelete="CASCADE"), unique=True, nullable=False)
...
class CaseMilestone(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    case_id = Column(UUID(as_uuid=True), ForeignKey("app.cases.id", ondelete="CASCADE"), nullable=False, index=True)
    milestone_type = Column(Enum(MilestoneType, name="milestone_type", schema="app", create_type=False), nullable=False)
...
class Notice(Base):
    ...
    created_by = Column(UUID(as_uuid=True), ForeignKey("app.users.id", ondelete="SET NULL"))
    status = Column(Enum(NoticeStatus, name="notice_status", schema="app", create_type=False), nullable=False, default=NoticeStatus.draft)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class NoticeDelivery(Base):
    ...
    channel = Column(Enum(DeliveryChannel, name="delivery_channel", schema="app", create_type=False), nullable=False)
    to_address = Column(String, nullable=False)
    provider_message_id = Column(String)
    status = Column(Enum(DeliveryStatus, name="delivery_status", schema="app", create_type=False), nullable=False, default=DeliveryStatus.queued)
...
class CaseRuleState(Base):
    ...
    case_id = Column(UUID(as_uuid=True), ForeignKey("app.cases.id", ondelete="CASCADE"), primary_key=True)
...
class Meeting(Base):
    ...
    meet_provider = Column(Enum(MeetProvider, name="meet_provider", schema="app", create_type=False), nullable=False, default=MeetProvider.google_meet)
    meet_url = Column(String)
    portal_url = Column(String)
    status = Column(Enum(MeetingStatus, name="meeting_status", schema="app", create_type=False), nullable=False, default=MeetingStatus.scheduled)
    notes = Column(Text)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
...
class Recording(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    meeting_id = Column(UUID(as_uuid=True), ForeignKey("app.meetings.id", ondelete="SET NULL"), index=True)
    case_id = Column(UUID(as_uuid=True), ForeignKey("app.cases.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    uploaded_by = Column(UUID(as_uuid=True), ForeignKey("app.users.id", ondelete="SET NULL"))
...
class Document(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    case_id = Column(UUID(as_uuid=True), ForeignKey("app.cases.id", ondelete="CASCADE"), nullable=False, index=True)
    uploaded_by_user_id = Column(UUID(as_uuid=True), ForeignKey("app.users.id", ondelete="SET NULL"))
    uploaded_by_victim_id = Column(UUID(as_uuid=True), ForeignKey("app.victim_accounts.id", ondelete="SET NULL"))
    source = Column(Enum(DocSource, name="doc_source", schema="app", create_type=False), nullable=False)
    category = Column(Enum(DocCategory, name="doc_category", schema="app", create_type=False), nullable=False, default=DocCategory.OTHER)
    file_name = Column(String, nullable=False)
    mime_type = Column(String)
    size_bytes = Column(BigInteger)
    storage_key = Column(String, nullable=False)
...
class DocumentAccessLog(Base):
    ...
    viewer_user_id = Column(UUID(as_uuid=True), ForeignKey("app.users.id", ondelete="SET NULL"), index=True)
...
class AuditLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    actor_type = Column(Enum(ActorType, name="actor_type", schema="app", create_type=False), nullable=False)
    actor_user_id = Column(UUID(as_uuid=True), ForeignKey("app.users.id", ondelete="SET NULL"))
    actor_victim_id = Column(UUID(as_uuid=True), ForeignKey("app.victim_accounts.id", ondelete="SET NULL"))
...
class CaseImport(Base):
    ...
    file_name = Column(String, nullable=False)
    storage_key = Column(String, nullable=False)
    status = Column(Enum(ImportStatus, name="import_status", schema="app", create_type=False), nullable=False, default=ImportStatus.uploaded)
...
class CaseImportRow(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    import_id = Column(UUID(as_uuid=True), ForeignKey("app.case_imports.id", ondelete="CASCADE"), nullable=False, index=True)
    row_no = Column(Integer, nullable=False)
    raw_data = Column(JSONB, nullable=False)
    case_id = Column(UUID(as_uuid=True), ForeignKey("app.cases.id", ondelete="SET NULL"))
    status = Column(Enum(ImportRowStatus, name="import_row_status", schema="app", create_type=False), nullable=False)
    error = Column(Text)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow, nullable=False)
