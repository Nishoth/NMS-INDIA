...
const MeetingRoom = () => {
  const { meetingId } = useParams();
  const navigate = useNavigate();
  const { handleRequest, getMeeting, uploadRecording } = useApi();
  const jitsiContainerRef = useRef(null);
  ...
  useEffect(() => {
    const fetchMeeting = async () => {
      const { data, error } = await handleRequest(() => getMeeting(meetingId));
      console.log("Meeting data:", data);
      if (error) {
        toast.error("Failed to load notice details");
      } else if (data) {
        setCaseId(data.case_id); // ✅ store it here
      }
    };
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      if (e.key === "PrintScreen" || e.keyCode === 44) {
        toast.error(
          "Screen capturing is strictly prohibited in this application."
        );
        navigator.clipboard.writeText(""); // Clear clipboard
      }
    };
  ...
  const handleUploadBlob = async (blob) => {
    setUploading(true);
    const file = new File([blob], `Meeting_${meetingId}_${Date.now()}.webm`, {
      type: "video/webm",
    });
    ...
    const targetCaseId = window.prompt(
      "Recording Saved! Enter tracking Case ID to bind this recording to:",
      caseId
    );

    if (targetCaseId) {
      const { error } = await handleRequest(() =>
        uploadRecording(targetCaseId, formData)
      );
      if (error) {
        toast.error("Failed to upload recording: " + error);
      } else {
  ...
  return (
    <div className="h-screen bg-[#1a1a1a] flex flex-col font-sans overflow-hidden relative select-none">
      ...
      <div className="fixed inset-0 pointer-events-none z-[9999] opacity-10 overflow-hidden flex flex-wrap content-center justify-center gap-20">
        {Array(20)
          ...
          .map((text, i) => (
            <div
              key={i}
              className="text-white text-3xl font-bold uppercase transform -rotate-45 whitespace-nowrap"
            >
              {text}
            </div>
          ))}
          ...
          <div className="relative border-l border-white/10 pl-4">
            {uploading ? (
              ...
              <button
                ...
                className="h-9 px-4 bg-red-600 hover:bg-red-700 text-white rounded-lg flex items-center gap-2 text-sm font-medium transition-colors shadow-lg shadow-red-600/20 animate-pulse"
              >
                <span className="w-2 h-2 rounded-full bg-white"></span> Stop
                Recording
              </button>
            ) : (
              <button
      ...
      <div className="bg-orange-500/10 border-b border-orange-500/20 px-6 py-2 flex items-center justify-center gap-2 text-xs text-orange-400 z-10 relative font-medium uppercase tracking-wider">
        <FiAlertTriangle />
        Do not attempt external screen capture. All streams are encrypted and
        tagged with digital watermarks.
      </div>
